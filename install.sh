...
config_after_install() {
    echo -e "${yellow}due to safety concerns，Install/After the update is complete, you need to force the port and account password to be changed.${plain}"
    ...
    if [[ x"${config_confirm}" == x"y" || x"${config_confirm}" == x"Y" ]]; then
        ...
        echo -e "${red}Cancelled,All settings are default,Please revise in time${plain}"
    fi
}
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}detect x-ui version failed，may be beyond Github API limit，Please try again later，or manually specify x-ui version installation${plain}"
            exit 1
        fi
    ...
    config_after_install
    #echo -e "If it is a fresh install，The default web port is ${green}54321${plain}，The username and password are both by default ${green}admin${plain}"
    #echo -e "Please make sure this port is not occupied by other programs，${yellow}and make sure 54321 port is released${plain}"
    #    echo -e "If you want to 54321 Change to another port，enter x-ui command to modify，Also make sure that the port you modify is also allowed"
    #echo -e ""
    ...
    echo -e "${green}x-ui v${last_version}${plain} The installation is complete，Panel is activated，"
    echo -e ""
    echo -e "x-ui How to use the management script: "
    echo -e "----------------------------------------------"
    echo -e "x-ui              - Show admin menu (function more Phien dich boi nguyen quoc anh)"
    echo -e "x-ui start        - Khoi dong x-ui panel"
    echo -e "x-ui stop         - stop x-ui panel"
    echo -e "x-ui restart      - restart x-ui panel"
    echo -e "x-ui status       - status x-ui state"
    echo -e "x-ui enable       - enable x-ui Auto-start"
    echo -e "x-ui disable      - disable x-ui Auto-start"
    echo -e "x-ui log          - nhat ky x-ui log"
